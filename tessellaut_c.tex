...
%We conjecture that given a cyclic cover branched over $\mathbb{C}\mathbb{P}^1,$ there exists a tessellation that exhibits the automorphism group of the curve. Further, 
...
\label{subsec:polynomial} Given brancing indices $d (d_i),$ we write $C$ as $y^d - (x - p_1)^{d_1} (x - p_2)^{d_1} \cdots (x - p_n)^{d_n}.$ When possible, we show that we can minimize the degree of the covering that still preserves the conformal structure. We verify with the program in \cite{jeroen} that the output (automorphism group) is independent of this change

\begin{exmp} ($g = 3$ Hyperelliptic curves)
In this paper, we look at three different hyperelliptic curves of genus three. % I'm going to include 7(1,1,5) in this example/discussion but let's not put it in the computation section. 
By different, we mean that the quotient spheres (under the hyperelliptic involution) are not conformally equivalent to each other. Instead of viewing them as $d$-fold cyclic covers over $n$-punctured spheres (where $n < 2 g + 2$), we view them as double covers over spheres branched at $2 g + 2$ points. The $2 g + 2$ points correspond to Weierstrass points (fixed points under the hyperelliptic involution), and the details can be found in section~\ref{subsec:Wpts}. 

Section 3.2 of \cite{dthesis} includes details regarding lifts of maps in particular. 

\begin{enumerate}
\item 8(1,1,6)

This case can be generalized to $d(1, 1, d-2)$ for any even $d.$ This is equivalent to a double-cover over a sphere where the branch points lie at the vertices of a double (regular) octagon. The eight ($= 2 g + 2$) points correspond to the Weierstrass points. The hyperelliptic involution is a lift of the involution $\phi$ on the thrice punctured sphere, where $\phi$ fixes $p_3$ and interchanges $p_1$ and $p_2.$ However, $p_3$ has two preimages so $\widetilde{\phi}$ interchanges $\widetilde{p_3}_1$ and $\widetilde{p_3}_2.$ In other words, none of the $\widetilde{p_i}$ are fixed under the hyperelliptic involution. The fixed points are $\widetilde{q}$ for $q \neq p_i$ for all $i,$ and we denote the plane curve model as $y^2 - (x^8 - 1)$ 


\item 12(1,5,6)

This case can be generalized to $d(1, \frac{d}{2} - 1, \frac{d}{2})$ for $d$ even. The quotient sphere (under the hyperelliptic involution) is a sphere where the branch points lie at the North pole, South pole, and six equidistributed points along the Equator. The hyperelliptic involution is a lift of an involution $\phi$ on the thrice punctured sphere, where $\phi$ similarly fixes $p_3$ and interchanges $p_1$ and $p_2.$ In this case, all $\widetilde{p_i}$ are fixed under the hyperelliptic involution and the plane curve model is $y^2 - x (x^{2 g} - 1).$


\item 7(1,1,5)
This case is similar to the first case, the difference comes from the parity of $d.$ If $d$ is odd, $\widetilde{p_3}$ and $\widetilde{q}$ (for $q \neq p_i,$ hence there are $d$ preimages of $q$) are fixed under the hyperelliptic involution. The quotient sphere is a cone over a regular $2 g + 1 (= d)$-gon, and the plane curve model is written as $y^2 - x(x^{2 g + 1} - 1).$


\item 6(1,3,5,3)
The last case is where we distribute the eight points at the vertices of a regular cube. This covering is shown in section 5.1 of the second author's thesis \cite{dthesis}. By stereographic projection map, we achieve $y^2 = x^8 - 14 x^4 + 1.$
\end{enumerate}
...
With branching indices $8 (1, 2, 5),$ the underlying curve of Octa-4 is written as $y^8 - x (x-1)^2.$ However, we can reduce the degree of the polynomial and write it as $y^4 - x (x-1) (x+1).$ See Theorem 5.1 from \cite{dami} for details.
...
%\begin{remark} \begin{dami} Why not ``These are maps that acts transitively and freely on the tiles.''\end{dami} In this case, all of the combinatorial automorphisms $\Aut(\Delta)$ lift to automorphisms of $C$, and every automorphism of $C$ is uniquely determined by its action described combinatorially.\end{remark}
...
%We checked the results programmatically with programs based around the algorithm of Bruin-Sijsling-Zotine \cite{jeroen}.
...
\begin{figure}[htbp]
    \centering
    \subfloat[The base tessellation $\Delta$]{{\includegraphics[width=2.75in]{figures/124_base_}}}
    \qquad
    \subfloat[The refined tessellation $\Delta_T$]{{\includegraphics[width=2.75in]{figures/124_hyp_}}}%
...
\begin{figure}[htbp]
    \centering
    \subfloat{{\includegraphics[width=2.75in]{figures/116_hyp}}}
...
\begin{dami} replace 12(1,5,6) with 8(1,3,4) bolza \end{dami}
...
The base tessellation of $12(1,5,6)$ is tiled by 16 $\frac{2 \pi}{12}$-hyperbolic triangles. However, to visualize the order-12 rotational symmetry, we subdivide four triangles into 12 third-triangles % (Figure~\crefformat{figure}{~#2#1{(a)}#3} \cref{fig:156}).
...
\begin{figure}[htbp]
    ...
    \qquad
    \subfloat[The refined tessellation $\Delta_T$]{{\includegraphics[width=2.75in]{figures/147_weight_}}}%
    \caption{12(1,4,7)}%
    \label{fig:147}%
\end{figure}
...
\bibitem{thesis} D. Lee, \textit{Geometric realizations of cyclically branched coverings over punctured spheres,} 2018, Ph.D thesis. 
